...
export default function ArsipInaktifPage() {
    const [filter, setFilter] = useState({ status: 'INAKTIF', page: 1, limit: 20, search: '' });
    ...
    return (
        <div>
            <div className="mb-6">
                <h1 className="text-2xl font-bold text-gray-900">Arsip Inaktif</h1>
                <p className="text-gray-500 mt-1">Arsip yang telah memasuki masa inaktif</p>
            </div>
            ...
            <div className="card overflow-x-auto">
                <table className="min-w-full divide-y divide-gray-200">
                    ...
                    <tbody className="divide-y divide-gray-200">
                        ...
                        {!isLoading && arsips.length === 0 && <tr><td colSpan={7} className="px-4 py-8 text-center text-gray-400">Tidak ada arsip inaktif</td></tr>}
                        {arsips.map((arsip: any) => (
                            <tr key={arsip.id} className="hover:bg-gray-50">
                                ...
                                <td className="px-4 py-3 text-sm text-gray-600">{arsip.tanggalInaktifBerakhir ? formatDate(arsip.tanggalInaktifBerakhir) : 'Permanen'}</td>
                                <td className="px-4 py-3"><span className={getStatusBadgeClass(arsip.status)}>{arsip.status}</span></td>
                                <td className="px-4 py-3">
                                    <Link href={`/arsip/inaktif/${arsip.id}`} className="text-primary-600 hover:underline text-sm">Detail</Link>
                                </td>
                            </tr>
                        ))}
                    </tbody>
